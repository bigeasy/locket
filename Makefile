...
sources = docco/locket.html css/style.css index.html
...
watch: all
	fswatch --exclude '.' --include '\.html$$' --include '\.less$$' --include '\.md$$' --include '\.js$$' pages css locket *.md | while read line; \
	do \
...
docco/%.html: locket/%.js node_modules/.bin/docco
	mkdir -p docco
	node_modules/.bin/docco -o docco -c docco.css locket/*.js
